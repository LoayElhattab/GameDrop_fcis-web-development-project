...
describe('Admin Controller (Admin Role Required)', () => {
    ...
    describe('updateUserRole', () => {
        test('should update a user\'s role successfully', async () => {
            const userIdToUpdate = 'user-1-uuid'; // A non-admin user
            const newRole = 'ADMIN';
            const updatedUser = { ...mockSingleUser, role: newRole };

            // Arrange: Mock Prisma to return the updated user
            mockPrisma.user.update.mockResolvedValue(updatedUser);

            const req = mockRequest({ role: newRole }, { userId: userIdToUpdate }); // New role in body, userId in params
            const res = mockResponse();
            const next = mockNext;
            ...
            // Assert: Verify Prisma update was called and response is as expected
            expect(mockPrisma.user.update).toHaveBeenCalledTimes(1);
            expect(mockPrisma.user.update).toHaveBeenCalledWith({
                where: { id: userIdToUpdate },
                data: { role: newRole.toUpperCase() }, // Controller converts to uppercase
                 select: { // Should match the select fields in the controller
                     id: true,
                     username: true,
                     email: true,
         ...
         test('should return 400 if user ID is missing in parameters', async () => {
             ...
             const req = mockRequest({ role: 'ADMIN' }, {}); // Missing userId in params
             const res = mockResponse();
             const next = mockNext;
             ...
             expect(error.message).toBe('User ID is required in parameters.'); // Match controller error
             expect(error.statusCode).toBe(400);
             expect(mockPrisma.user.update).not.toHaveBeenCalled(); // Should not call Prisma
             expect(res.status).not.toHaveBeenCalled();
             expect(res.json).not.toHaveBeenCalled();
         });

         test('should return 400 if new role is missing in body', async () => {
             // Arrange: Missing role in req.body
             const req = mockRequest({}, { userId: 'user-1-uuid' }); // Missing role in body
             const res = mockResponse();
             const next = mockNext;
             ...
             expect(next).toHaveBeenCalledTimes(1);
             const error = next.mock.calls[0][0];
             expect(error.message).toBe('New role is required in body.'); // Match controller error
             expect(error.statusCode).toBe(400);
              expect(mockPrisma.user.update).not.toHaveBeenCalled(); // Should not call Prisma
             expect(res.status).not.toHaveBeenCalled();
             expect(res.json).not.toHaveBeenCalled();
         ...
         test('should return 400 if new role is invalid', async () => {
              // Arrange: Invalid role value in req.body
              const req = mockRequest({ role: 'SUPERADMIN' }, { userId: 'user-1-uuid' }); // Invalid role
              const res = mockResponse();
              const next = mockNext;
              ...
              // Assert: Verify next was called with a 400 error and correct message
              expect(next).toHaveBeenCalledTimes(1);
              const error = next.mock.calls[0][0];
              expect(error.message).toContain('Invalid role "SUPERADMIN". Allowed roles are: CUSTOMER, ADMIN'); // Match controller error
              expect(error.statusCode).toBe(400);
              expect(mockPrisma.user.update).not.toHaveBeenCalled(); // Should not call Prisma
              expect(res.status).not.toHaveBeenCalled();
              expect(res.json).not.toHaveBeenCalled();
         ...
         test('should return 403 if admin tries to change their own role', async () => {
              // Arrange: Admin user ID matches the userId in params
              const req = mockRequest({ role: 'CUSTOMER' }, { userId: 'test-admin-uuid' }); // Admin changing own role
              const res = mockResponse();
              const next = mockNext;
              ...
              expect(next).toHaveBeenCalledTimes(1);
              const error = next.mock.calls[0][0];
              expect(error.message).toBe('Cannot change your own role via this endpoint.'); // Match controller error
        ...
        test('should return 404 if user to update is not found (Prisma P2025)', async () => {
            ...
            const userIdToUpdate = 'non-existent-uuid';
            const req = mockRequest({ role: 'CUSTOMER' }, { userId: userIdToUpdate });
            const res = mockResponse();
            const next = mockNext;
            ...
            // Assert: Verify Prisma update was called, next was called with a 404 error
            expect(mockPrisma.user.update).toHaveBeenCalledTimes(1);
             expect(mockPrisma.user.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: userIdToUpdate } })); // Check where clause
            expect(next).toHaveBeenCalledTimes(1);
            const error = next.mock.calls[0][0];
            expect(error.message).toBe(`User with ID ${userIdToUpdate} not found.`); // Match controller's wrapped error
            expect(error.statusCode).toBe(404);
            expect(res.status).not.toHaveBeenCalled();
            expect(res.json).not.toHaveBeenCalled();
        });
        ...
        test('should call next with error for other database errors', async () => {
             ...
             const userIdToUpdate = 'user-1-uuid';
             const req = mockRequest({ role: 'ADMIN' }, { userId: userIdToUpdate });
             const res = mockResponse();
             const next = mockNext;
             ...
             // Assert: Verify Prisma update was called, next was called with the original error
             expect(mockPrisma.user.update).toHaveBeenCalledTimes(1);
             expect(next).toHaveBeenCalledWith(dbError);
             expect(res.status).not.toHaveBeenCalled();
             expect(res.json).not.toHaveBeenCalled();
